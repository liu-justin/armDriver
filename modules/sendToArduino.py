...
def sendToArduino(timeR, stepR, timeC, stepC):
	...
	for i in range(len(timeR)-1):
		...
		stepDirection = np.sign(stepR[i+1] - stepR[i]) + 126 # plus 102 to get 101,102,103, bytes that i allocated for stepDirection
		ser.write((stepDirection).to_bytes(1, byteorder="big"))

	# byte to start reading motorC
